...
module.exports = class extends Command {
    ...
    const categoryHelp = [];
        ...
        categoryHelp.push(`= ${subCategories[subCat]} =\n${help[categories[cat]][subCategories[subCat]].join('\n')}\n`);
        display.addPage(template => template.addField(`**${categories[cat]} Commands**`, `${"```asciidoc"}\n${/*categoryHelp[subCat]*/`= ${subCategories[subCat]} =\n${help[categories[cat]][subCategories[subCat]].join('\n')}\n`}\n${"```"}`));
      }
			// helpMessage.push('```\n\u200b');
		}
